...
SQL is a language for making queries into a database whereas RDBMS is the basis for SQL and what most databases are based on today.

1. Why do tables need a `primary key`?

You need a primary key to make sure that each row in a database has a unique identifier and can be compared without worrying about it having similar data to other entries.
...
You need a third table which uses a composite key to connect them. Set's up a one to many relationship to each table and works together to allow the many to many relationship.
